...
const PARAMS = {
  ...
  // 조향력
  SEPARATION_FORCE: 8.2, // 분리 반발 강도
  COHESION_FORCE: 5.2, // 중심 응집 강도
  ALIGNMENT_FORCE: 0.8, // 방향 정렬 강도
  PUSH_WEIGHT: 0.06, // 전체 push 적용 배율
  TURN_RESPONSE: 1.75, // 평상시 선회 반응 배수
  // 뒤처짐 가속
  LAG_THRESHOLD_RATIO: 1.2, // 가속 시작 거리 (스프라이트 크기 배수)
...
export function applySpriteBat(el, animal) {
  ...
  if (animal.batType) {
    const next = "sprite_bat " + animal.batType;
...
export function applyHomeGroupBat(groupIds, animalsRef, rect) {
  ...
  if (coreIds.length >= 2) {
    coreIds.forEach((id) => {
      ...
      animal.vx += pushX * PARAMS.PUSH_WEIGHT * PARAMS.TURN_RESPONSE;
      animal.vy += pushY * PARAMS.PUSH_WEIGHT * PARAMS.TURN_RESPONSE;
      ...
      if (rect) {
        ...
        animal.vx += steerX * PARAMS.TURN_RESPONSE;
        animal.vy += steerY * PARAMS.TURN_RESPONSE;
...
export default {
  ...
  homeInit: initHomeBat,
  homeGroup: applyHomeGroupBat,
};
