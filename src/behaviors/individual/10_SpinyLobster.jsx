...
const PARAMS = {
  ...
  START_GAP_RATIO: 0.8, // 초기 배치 간격 배수
  POINT_INTERVAL: 50, // 선두 개체 포인트 기록 주기
  POINT_REACHED_DISTANCE: 10, // 후발 개체 포인트 도달 판정 거리
  STEER_BLEND: 0.08, // 기본 조향 보간 비율
  LEADER_RETURN_BLEND: 0.008, // 선두 복귀 조향 비율
  ...
  STOPPED_DISTANCE: 0.01, // 일반 개체 정지 판정 거리
...
function compareQueueLength(leftAnimal, rightAnimal) {
  const leftStopped = isStoppedAnimal(leftAnimal) ? 1 : 0;
  const rightStopped = isStoppedAnimal(rightAnimal) ? 1 : 0;
  if (leftStopped !== rightStopped) {
    return leftStopped - rightStopped;
  }

  const leftQueue = leftAnimal?.pointQueue?.length || 0;
  const rightQueue = rightAnimal?.pointQueue?.length || 0;
...
function applyWallAvoidance(animal, rect) {
  // 진행 방향 앞쪽을 미리 보고 벽에 가까우면 목표 방향을 서서히 꺾는다.
...
function getLeaderAnimal(orderedIds, animalsRef) {
  // 기존 선두를 유지하되, 없으면 첫 개체를 선두로 승계한다.
...
function appendLeaderPointToQueues(orderedIds, animalsRef, point) {
  // 선두가 찍은 포인트를 멈추지 않은 후발 개체들의 큐에만 추가한다.
  orderedIds.forEach((id) => {
    const animal = animalsRef.current[id];
    if (!animal) return;
...
function steerAnimalTowardDirectionWithBlend(
  ...
  // 현재 방향과 목표 방향을 blend 비율로 섞어 부드럽게 조향한다.
...
function updateLobsterRotation(animal) {
...
function getLeaderAvoidDirection(animal, rect) {
  // 선두는 전방 감지 지점이 벽 여유 범위에 들어오면 화면 안쪽 목표로 미리 튼다.
...
function updateLeaderPoints(leader, orderedIds, animalsRef) {
  // 선두는 일정 주기마다 현재 위치를 기록해 후발 개체들의 경로 큐를 만든다.
...
function consumeReachedPoint(animal) {
  // 현재 목표 포인트에 충분히 가까워지면 다음 포인트로 넘어간다.
  ensurePointQueue(animal);
...
export function applySpriteSpinyLobster(el, animal) {
  const sprite = el.querySelector(".sprite_spiny_lobster");
  if (!sprite) return;
  sprite.style.transform = `rotate(${animal.rotation}deg) scaleX(${animal.scaleX})`;
  if (animal.lobsterType) {
    const next = "sprite_spiny_lobster " + animal.lobsterType;
    if (sprite.className !== next) sprite.className = next;
  }
}

export function initHomeSpinyLobster({
  animal,
  speciesId,
  speciesAnchors,
  width,
...
export function applyHomeGroupSpinyLobster(groupIds, animalsRef, rect) {
  ...
  orderedIds.forEach((id) => {
    ...
    updateLobsterRotation(animal);
  });
}

export default {
  init: initSpinyLobster,
  update: updateSpinyLobster,
  applySprite: applySpriteSpinyLobster,
  homeInit: initHomeSpinyLobster,
  homeGroup: applyHomeGroupSpinyLobster,
};
