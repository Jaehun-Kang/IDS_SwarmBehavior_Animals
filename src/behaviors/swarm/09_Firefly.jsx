...
import {
  drawFlashlightOverlay as drawSharedFlashlightOverlay,
  FLASHLIGHT_PRESET,
...
const PARAMS = {
  ...
  FLASHLIGHT_PULSE_STRENGTH: 30,
  FLASHLIGHT_PULSE_RADIUS_PX: 148,
...
const drawFlashlightOverlay = (ctx, pointerState, width, height) => {
  drawSharedFlashlightOverlay(ctx, pointerState, {
    width,
    height,
    ...FLASHLIGHT_PRESET,
  });
  ...
  React.useEffect(() => {
    ...
    const render = (timestamp) => {
      if (!shouldRenderFrame(isPaused, window.innerWidth, window.innerHeight,
        window.devicePixelRatio || 1, sanitizedControls, rasterCanvasRef.current, frameCanvasesRef.current)) {
