  ...
  React.useEffect(()=>{
    ...
    const loop=createBookCanvasLoop(canvasRef.current,{
      ...
      onFrame:({context:ctx,width,height,elapsedSeconds})=>{
        ...
        ctx.strokeStyle="#a64046";ctx.lineWidth=2;
        ctx.beginPath();ctx.arc(x*s,y*s,0.65*s,0,Math.PI*2);ctx.stroke();
        ctx.beginPath();ctx.moveTo((x-0.3)*s,(y-0.3)*s);ctx.lineTo((x+0.3)*s,(y+0.3)*s);
        ctx.moveTo((x+0.3)*s,(y-0.3)*s);ctx.lineTo((x-0.3)*s,(y+0.3)*s);ctx.stroke();
        for(let i=0;i<model.agents.length;i++){
          ...
          const frame=warning?stage.bee_top_fly.frames[Math.floor(model.time*16+a.id)%2]:stage.bee_top_idle.frame;
          const size=s*0.95;
          ctx.save();ctx.translate(pose.x*s,pose.y*s);
          ctx.rotate(pose.heading+(warning?Math.sin(model.time*20+a.id*0.3)*0.12:0));
