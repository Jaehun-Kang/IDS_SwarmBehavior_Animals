...
import { drawFlowMarkers } from "./bookEnvironmentDrawing.js";
import { createBeeFanningFlow, advanceBeeFanningFlow } from "./beeFanningFlow.js";
...
export default function BeeFanningPreview({controls,ruleGroup}){
  ...
  React.useEffect(()=>{
    let model,frames,flow,disposed=false,stillFrames=0;
    const loop=createBookCanvasLoop(canvasRef.current,{
      onResize:({width,height})=>{model=createBeeFanning(width/height);flow=createBeeFanningFlow(width,height);stillFrames=0;},
      onFrame:({context:ctx,width,height,elapsedSeconds})=>{
        ...
        const flowSpeed=advanceBeeFanningFlow(flow,model.agents,s,elapsedSeconds);
        stillFrames=model.agents.some(a=>a.fanning)||flow.markers.length?0:stillFrames+1;
        ...
        drawFlowMarkers(ctx,flow,Math.PI,flowSpeed,p=>Math.max(0,1-p.age/6));
        for(const a of model.agents){
          const stage=atlas.stages;
          const frame=a.fanning?stage.bee_top_fly.frames[Math.floor(model.time*20+a.id)%2]:stage.bee_top_idle.frame;
          const size=s*1.15;
